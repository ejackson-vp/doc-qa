...
import { generateLimiter } from '@/app/lib/concurrency-limiter';
...
export async function POST(
  ...
  try {
    ...
    // Generate answer with concurrency limiting
    // If at capacity, the request will be queued automatically
    const result = await generateLimiter.run(async () => {
      return await processAnswerGeneration(
        id, 
        user_id,
        content_prompt, 
        format_prompt, 
...
async function processAnswerGeneration(
  id: string,
  user_id: string,
  content_prompt: string | undefined,
  format_prompt: string | undefined,
  ...
  // Return generation info for client to store
  ...
  return {
    ...
    completed_at: finalData.status === 'completed' ? new Date().toISOString() : undefined,
  };
}
